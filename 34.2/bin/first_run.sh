...
# Download last obiba mica version from Drupal
	cd /var/www/html && \
	drush -y dis obiba_mica && \
	drush -y dis obiba_agate && \
	drush -y dis obiba_bootstrap && \
	drush -y dl obiba_mica && \
	drush -y dl obiba_agate && \
	drush -y dl obiba_bootstrap
